...
// Getting Data from branch table in member
$sql = "SELECT * FROM memberform";
    ...
    <div class="container mt-5">
        ...
        <div class="table-responsive">
                    ...
                    if (mysqli_num_rows($result) == !empty($result)) {
                        ...
                        foreach ($allRows as $row) { ?>
                            <tr>
                                ...
                                <td><?php echo $row["member_address"] ?>
                                <td><?php echo $row["mobile_number"] ?>
                                <td><?php echo $row["branch_name"] ?>
                                <td><?php echo $row["member_zone"] ?>
                                </td>
                                <td class='d-flex'> <a target="_blank" class="btn btn-outline-primary w-50 me-2"
                                        href="member_form.php?update_id=<?php echo $row['id']; ?>">UPDATE</a>
                                    <a class="btn btn-danger w-50"
                                        href="member_table.php?delete_id=<?php echo $row['id']; ?>">DELETE</a>
                                </td>
                            </tr>
                        <?php }
                    } else {
            ...
            if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
                ...
                $sql = "DELETE FROM memberform WHERE id=$delete_id";
